...
class ExcelReportController extends Controller
{
    ...
    public function excel_producto($param){
        ...
        if ( $idcat != '' && $idsuc != '') {
            // $query = trim($request->get('searchText'));
               ...
               // ->where('p.precio','LIKE','%'.$precio.'%')
                ...
                ->select('p.idproducto as ID','p.codigo','p.fechaCod as FechaCod','co.nombre as Color','ta.nombre as Talla','t.nombre as Tela','p.precio as PrecioInicial','c.nombre as Categoria','s.nombre as Sucursal','p.lote as Lote')
        ...
        Excel::create('Productos', function($excel) use($data) {
            ...
            return redirect()->back()->withErrors('Antes tiene que hacer una busqueda: Recuerde que solo genera el repote de sucursal y categoria ya definida...');
        }
