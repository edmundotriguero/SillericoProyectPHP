...
<div class="row">
		...
		<div class="table-responsive">
			<table class="table table-striped table-bordered table-condensed table-hover">
				...
				<tr>
					...
					<td>{{ $mov->categoria}}</td>
					<td>{{ $mov->codigo}}</td>
					@foreach($sucursales as $suc) @if($suc->idsucursales == $mov->idSucOrigen)
					<td>{{ $suc->nombre}}</td>

					@endif @endforeach @foreach($sucursales as $suc) @if($suc->idsucursales == $mov->idSucDestino)
					<td>{{ $suc->nombre}}</td>
	...
	@include('almacen.movimiento.modal') @endforeach
			</table>
		</div>
		{{$movimientos->render()}}
	</div>
